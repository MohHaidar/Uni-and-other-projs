...
Q = 0.0001;                     %Process variance (controls the change in the filtered signal, how muchthe process model is trusted) 
...
H(1) = 0.0003;                  %The error margin on initial state considered as first measurement

%% Kalman process
for i=2:length(avrg)
    
    %predict
    P_(i) = P(i-1)+(x(i)-x(i-1))^2/0.1/33000;                      %  x_[k+1] = A.x[k] + B.u[k]  (A = 1, B = 0)    Predicted (a priori) state estimate 
