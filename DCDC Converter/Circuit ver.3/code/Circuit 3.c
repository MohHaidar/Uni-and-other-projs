...
void main() {
     ...
     ADCON1 = 0b11010011;    //2's complement result , Fosc/16 , Vref- = Vss , Vref+ = FVR
     ...
     //Set period: PSMC1PR = 799 /// period = PSMC1PR + 1 / FPSMC_clk(=FOSC)
     PSMC1PRH = 0x01;
     PSMC1PRL = 0x8F;
...
float read_AC(){
  ...
  int i;
    ...
    read_time = 4.48/1000.0;  // max read time in s
    f_min = 10.0;
    t_max = 1/f_min;
...
//// ******************** All tunable variables ******************** ////
     ...
     D[0] = 25;        //25 to 150 for 4MHz,   10KHz PWM
     step = 5;
     thr = 5.0;
     dD = step;
     m=2;
              ...
              if((dP<thr)&&(dP>0.0))
                  ...
                  if((dP>-thr)&&(dP<=0.0)){
                      ...
                      if(dP<0.0)              //normal case reverse the step if dP is negative
                          dD = -dD;
// End algo
          if(D[1] + dD > 150)              //keep the duty cycle values inside the limits
               dD = -step;
          if(D[1] + dD < 25)
               dD = step;
     }
     else{
          D[1] = 25;                       //resetting the algorithm
          dD = step;
     }
