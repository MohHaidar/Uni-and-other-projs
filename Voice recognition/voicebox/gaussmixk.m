...
%0002 %GAUSSMIXK approximate Kullback-Leibler divergence between two GMMs + derivatives
%0003 %
%0004 % Inputs: with kf & kg mixtures, p data dimensions
%0005 %
%0006 %   mf(kf,p)                mixture means for GMM f
%0007 %   vf(kf,p) or vf(p,p,kf)  variances (diagonal or full) for GMM f
%0008 %   wf(kf,1)                weights for GMM f - must sum to 1
%0009 %   mg(kg,p)                mixture means for GMM g [g=f if mg,vg,wg omitted]
%0010 %   vg(kg,p) or vg(p,p,kg)  variances (diagonal or full) for GMM g
%0011 %   wg(kg,1)                weights for GMM g - must sum to 1
%0012 %
%0013 % Outputs:
%0014 %   d             the approximate KL divergence D(f||g)
%0015 %   klfg(kf,kg)   the exact KL divergence between the components of f and g
%0016 %
...
%0019 % It is defined as <log(f(x)) - log(g(x))> where <y(x)> denotes the
...
%0029 % if f=g. However, the approximation may be negative if f and g are different.
%0030 %
%0031 % Refs:
